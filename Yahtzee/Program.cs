...
namespace Yahtzee
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            int[] compScorecard = new int[TOTAL + 1];
            int userMoves = 0;
            int compMoves = 0;
            bool userTurn = false;
            userScorecard = ResetScorecard(userScorecard);
            compScorecard = ResetScorecard(compScorecard);
            //set window size ~50 lines
            do
            {
                userTurn = true;
                userScorecard = UpdateScorecard(userScorecard);
                compScorecard = UpdateScorecard(compScorecard);
            ...
            while (userCount >= YAHTZEE && compCount >= YAHTZEE);
            userScorecard = UpdateScorecard(userScorecard);
            compScorecard = UpdateScorecard(compScorecard);
            DisplayScorecards(userScorecard, compScorecard);
            Console.WriteLine("X WON");
            List<int> dice = new List<int>() { 2, 3, 3, 6, 6 };
        ...
        static int[] ResetScorecard(int[] scorecard)
        {
            //i dont understand how to alter and return two different values from one method
            foreach (int s in scorecard)
                scorecard[s - 1] = -1;
            return scorecard;
        }
        ...
        static int[] UpdateScorecard(int[] scorecard)
        {
        ...
        static List<int> Roll(int rolls, List<int> dice)
        {
            Random d6Roll = new Random();
            for (int i = 0; i < rolls; i++)
                dice[i] = d6Roll.Next(0, 7);
            return dice;
            ...
            static int GetComputerScorecardItem(int[] scorecard, List<int> dice)
            {
                int indexOfMax = 0;
                int max = 0;
                ...
                static void ComputerPlay(int[] cScorecard, int[] counts)
                {
                ...
                static void GetKeeping(List<int> dice, List<int> keeping)
                {
                }
                ...
                static int Score(List<int> dice, int scorecard)
                {
